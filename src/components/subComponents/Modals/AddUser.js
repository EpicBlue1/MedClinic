...
const AddUser = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (UserType === "Patient") {
      if (result) {
        ...
        axios
          .post("http://localhost/MedClinic_TermTwo/addPat.php", addData)
      ...
      if (result) {
        ...
        axios
          .post("http://localhost/MedClinic_TermTwo/addDoc.php", addData)
          .then(function (response) {
            if (response) {
              console.log(response);
  ...
  return (
    <form
      ...
      className={props.Active ? "hide" : "UpdateUser borderRad"}
    >
      <h2>Add {UserType}</h2>
      <input
        onChange={FirstOnChange}
        className="AddInput borderRad"
        required
        id="First"
      ...
      />
      <input
        onChange={PropOneOnChange}
        className="AddInput borderRad"
        required
        id="PropertyOne"
        placeholder={props.PropertyOne}
      />
      <input
        onChange={PropTwoOnChange}
        className="AddInput borderRad"
        type="number"
        maxlength="9"
        required
        id="PropertyTwo"
        placeholder={props.PropertyTwo}
      />
      <input
      ...
      />
      <button type="submit" className="AddUserButton borderRad cursor">
        Add
      </button>
      <div onClick={closeModal} className="AddUserButton borderRad cursor">
        Close
      </div>
    </form>
  );
