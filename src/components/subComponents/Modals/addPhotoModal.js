...
const AddPhotoModal = (props) => {
    ...
    <div className={props.openModal ? "hide" : "AddProfile"}>
        <div className="imageArea borderRad">
            ...
            <input name="imageUrl" className='imgInput' id="imgInput" type="file" onChange={imageVal} />
            <div onClick={addProfile} className="LogButton cursor addImageButton">Add Profile</div>
            <div onClick={CloseNReset} className="LogButton borderRad">Cancel</div>
        </div>
    </div>
    );
};
